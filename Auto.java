	...
	public void auto() {
			for(int i =0; i<3; i=i) {
				if (square){//do this code if the goal is a square, sequence will repeat 4 times
					if(sensors.encR.get() <= 15 && sensors.encL.get() <= 15) {
						drive.move(STANDARD_SPEED);}// if the rotations are less than 15 move at standard speed
					if (sensors.encR.get() >= 15 && sensors.encL.get() >= 15) {
						drive.stop();
						forward = true;}// if the rotations are more than 15 stop and forward = true
			...
			for(int i = 0; i<2; i=i) {
				if (triangle){// do this is the goal is a triangle, sequence will repeat 3 times
					if(sensors.encR.get() <= 15 && sensors.encL.get() <= 15) {
						drive.move(STANDARD_SPEED);}// if there is less than 15 robot will srive forward
					if (sensors.encR.get() >= 15 && sensors.encL.get() >= 15) {
						drive.stop();
						forward = true;}// when the rotations are more than 15 robot stops and forward = true
